...
namespace JobFair.Forms.Recruiter
{
    public partial class ViewQuestionnarie : System.Web.UI.Page
    {
        ...
        {
            DataSet dsQuestionnarie = new DataSet();

            QuestionnarieREBAL questionnarieBAL = new QuestionnarieREBAL();
            dsQuestionnarie = questionnarieBAL.ViewQuestionnarieBAL(questionnarieId);
            // Check if dsQuestionnarie is not null
            if (dsQuestionnarie != null)
            {
                foreach (DataRow item in dsQuestionnarie.Tables[0].Rows)
                {
